...
(define-data-var net-asset-value uint u0) ;; NAV in uSTX per full PoolMind token
...
(define-data-var token-name (string-utf8 32) u"PoolMind")
(define-data-var token-symbol (string-utf8 10) u"PLMD")
...
;; @returns (response (string-utf8 32))
(define-read-only (get-name)
  (ok (var-get token-name))
)

;; @desc Gets the symbol of the fungible token.
;; @returns (response (string-utf8 10))
