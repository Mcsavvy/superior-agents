...
import { getUserProfile, removeAuthToken, unlinkWalletAddress, getUserBalances } from "../utils/api";
...
const Profile: React.FC = () => {
  ...
  const formatBalance = (balance: TokenBalance, tokenSymbol: string) => {
    ...
    if (tokenSymbol === 'STX') {
      // Format STX with up to 6 decimal places, remove trailing zeros
      return amount.toLocaleString('en-US', {
        minimumFractionDigits: 0,
        maximumFractionDigits: 6,
      });
    } else if (tokenSymbol === 'PLMD') {
      // Format PLMD with up to 6 decimal places, remove trailing zeros
      return amount.toLocaleString('en-US', {
        minimumFractionDigits: 0,
        maximumFractionDigits: 6,
      });
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center p-5 bg-gradient-to-br from-[#667eea] to-[#764ba2] font-sans">
        <div className="bg-white rounded-2xl p-8 shadow-2xl max-w-lg w-full text-center">
          <h2 className="mb-4 text-gray-800 text-2xl font-semibold">❌ Error</h2>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-5 bg-gradient-to-br from-[#667eea] to-[#764ba2] font-sans">
      <div className="bg-white rounded-2xl p-8 shadow-2xl max-w-lg w-full">
        <h2 className="mb-6 text-gray-800 text-2xl font-semibold text-center">👤 User Profile</h2>
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="space-y-2 text-sm">
              ...
              <p><span className="font-medium">Telegram ID:</span> {user.telegramId}</p>
            </div>
          </div>

          <div className="bg-gray-50 p-4 rounded-lg">
            <h3 className="font-semibold text-gray-700 mb-2">Wallet & Verification</h3>
            <div className="space-y-2 text-sm">
              {user.walletAddress ? (
          ...
          {user.walletAddress && (
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="font-semibold text-gray-700 mb-3">Wallet Balances</h3>
              {balanceLoading ? (
                ...
                <div className="space-y-3">
                  {balances.stx && (
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <img src="/tokens/stx.png" alt="STX" className="w-6 h-6" />
                        <span className="font-medium">STX</span>
                      </div>
                      <span className="font-mono text-sm">{formatBalance(balances.stx, 'STX')}</span>
                    </div>
                  )}
                  {balances.plmd && (
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <img src="/tokens/plmd.png" alt="PLMD" className="w-6 h-6" />
                        <span className="font-medium">PLMD</span>
                      </div>
                      <span className="font-mono text-sm">{formatBalance(balances.plmd, 'PLMD')}</span>
                    </div>
                  )}
                </div>
              ) : (
